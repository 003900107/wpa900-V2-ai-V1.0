...
void FUN_FFT_64(int phase,MEA_DFT *FFT)		 //64点FFT
{
  ...
  //double temp, temp2;
  /*
  temp = log(SAMP_POINT_NBR);
  temp2 = log(2);
  k = (int)(temp/temp2); 
  */
  
  k=log(SAMP_POINT_NBR)/log(2);
...
void TOTAL_MEASURE(Mea_Para *measure)
{
  ...
  (measure->mua[1].phase)-=4.5;                    /*弥补硬件A相电压相移滞后4.5°*/
...
void ValueScaling(float *pMeaTab,Mea_Para *measure)
{
  ...
  /*1*/*(pMeaTab++)=measure->mua[1].am*SetCurrent.ChannelCoef[0]*3/53;
  /*2*/*(pMeaTab++)=measure->mua[1].phase;
  /*3*/*(pMeaTab++)=measure->mub[1].am*SetCurrent.ChannelCoef[1]*3/53;
  /*4*/*(pMeaTab++)=measure->mub[1].phase;
  /*5*/*(pMeaTab++)=measure->muc[1].am*SetCurrent.ChannelCoef[2]*3/53;
  /*6*/*(pMeaTab++)=measure->muc[1].phase;
  
  /*7*/*(pMeaTab++)=measure->mia[1].am*SetCurrent.ChannelCoef[3]*1/200;
  /*8*/*(pMeaTab++)=measure->mia[1].phase;
  /*9*/*(pMeaTab++)=measure->mib[1].am*SetCurrent.ChannelCoef[4]*1/200;
  /*10*/*(pMeaTab++)=measure->mib[1].phase;
  /*11*/*(pMeaTab++)=measure->mic[1].am*SetCurrent.ChannelCoef[5]*1/200;
  /*12*/*(pMeaTab++)=measure->mic[1].phase;
  
  /*13*/*(pMeaTab++)=measure->muab[1].am*SetCurrent.ChannelCoef[6]*3/53;
  /*14*/*(pMeaTab++)=measure->mubc[1].am*SetCurrent.ChannelCoef[7]*3/53;
  /*15*/*(pMeaTab++)=measure->muca[1].am*SetCurrent.ChannelCoef[8]*3/53;
  if(*pBaseMeaTab>10.0)
